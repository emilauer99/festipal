...
describe('resolveColdStartRedirect (first-login-unmatched-route)', () => {
  it('THE regression — empty state (no href, no slug) resolves to Home, not a silent fall-through', () => {
    // Fresh account, no saved festival, no deep link: the cold-start effect
    // MUST navigate explicitly to /home. Previously this branch did nothing,
    // leaving the router on the unmatched root path `/` → Unmatched Route.
    expect(resolveColdStartRedirect(null, undefined)).toEqual({ kind: 'home' });
  });
  ...
  it('an empty-string href is not treated as a deep link (falls through to slug/home)', () => {
    expect(resolveColdStartRedirect('', undefined)).toEqual({ kind: 'home' });
  ...
  it('an empty-string slug is not treated as an active festival (falls through to home)', () => {
    expect(resolveColdStartRedirect(null, '')).toEqual({ kind: 'home' });
  });
});
...
describe('coldStartRedirectHref (first-login-unmatched-route round 2)', () => {
  ...
  // pinned explicitly.
  it('maps the empty-state home target to /home', () => {
    expect(coldStartRedirectHref({ kind: 'home' })).toBe('/home');
  });
  ...
  it('THE regression, end to end — fresh account (no href, no slug) redirects `/` to /home', () => {
    expect(coldStartRedirectHref(resolveColdStartRedirect(null, undefined))).toBe('/home');
  });
});
