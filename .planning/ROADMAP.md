...
**Plans**: TBD
**Notes**: HIGH-priority research flag — the community NestJS wrapper (`@thallesp/nestjs-better-auth`) × ts-rest middleware ordering is the single riskiest wiring point; spike early and keep a hand-rolled `@All('auth/*path')` catch-all documented as the fallback. Addresses Pitfalls 3 (AuthGuard mis-tagging), 4 (gate-less entry ≠ dropping `festivalId` isolation), 8 (OTP rate-limit/enumeration), and 11 (username race).
